 ...
 */
char *locates_command(char *cmd, char **envir_var)
{
	...
	path = get_envir_var("PATH", envir_var);
	if (path)
	{
		path_cpy = _strgdup(path);
		cmd_length = _strglen(cmd);
		path_token = _strgtok(path_cpy, ":");
		i = 0;
		while (path_token != NULL)
		{
			if (is_colon_cdir(path, &i))
				if (stat(cmd, &file_stat) == 0)
					return (cmd);
			dir_length = _strglen(path_token);
			directory = malloc(dir_length + cmd_length + 2);
...
int exec_cmd(data_shell *data_sh)
{
	...
	if (exec_index == 0)
	{
		ddir = locates_command(data_sh->args[0], data_sh->envir_var);
	...
	if (processID == 0)
	{
		if (exec_index == 0)
			...
			ddir = data_sh->args[0];
		execve(ddir + exec_index, data_sh->args, data_sh->envir_var);
	}
	else if (processID < 0)
	{
